...
class PriorityAgent():
    ...
    def step(self, state, action, reward, next_state, done):
        # Save experience in replay memorys
    ...
    def learn(self, gamma):
        ...
        # Double DQN
        Q_next_index   = self.qnetwork_local(next_states).detach().max(1).indices # get best actions
        Q_targets_next = self.qnetwork_target(next_states).detach().gather(1, Q_next_index.view(-1,1)) # get values
        ...
        loss  = (Q_targets.detach() - Q_expected).pow(2).squeeze(1) * weights
        prios = loss + 1e-5
        loss  = loss.mean()
